    ...
    const std::string& rootFrameName)
{
    ...
    if (!file.is_open() || !file.good()) {
        throw std::runtime_error(
            "leph::Model: Unable to load URDF file: " 
            + filename);
    }
    ...
    const std::string& rootFrameName)
{
    ...
    if (!urdf_model.initString(fileContent)) {
        throw std::runtime_error(
            "leph::Model: Unable to load URDF content.");
    }
    ...
    if (
        ...
        throw std::runtime_error(
            "leph::Model: Unable to load model from URDF.");
    }
    ...
    if (modelTmp.mBodyNameMap.count(rootFrameName) == 0) {
        throw std::logic_error(
            "leph::Model: Root body name unknown: " 
            + rootFrameName);
    }
...
//if ROS version >= ROS_MELODIC
    ...
    for (size_t i=0;i<indexesNewToOld.size();i++) {
        if (
            ...
            throw std::logic_error(
                "leph::Model: Inconsistency in RBDL joint index mapping");
        }
...
unsigned int Model::getIndexDOF(const std::string& name) const
{
    if (getMappingDOFs().count(name) == 0) {
        throw std::logic_error(
            "leph::Model::getIndexDOF: Invalid DOF name: " 
            + name);
    }
...
unsigned int Model::getIndexJoint(const std::string& name) const
{
    if (getMappingDOFs().count(name) == 0) {
        throw std::logic_error(
            "leph::Model::getIndexJoint: Invalid DOF name: " 
            + name);
    }
    size_t index = getMappingDOFs().at(name);
    if (index < 6 || index-6 >= sizeJoint()) {
        throw std::logic_error(
            "leph::Model::getIndexJoint: Invalid joint name: " 
            + name);
    }
    return index-6;
}
unsigned int Model::getIndexFrame(const std::string& name) const
{
    if (getMappingFrames().count(name) == 0) {
        throw std::logic_error(
            "leph::Model::getIndexFrame: Invalid frame name: " 
            + name);
    }
...
const std::string& Model::getNameDOF(size_t index) const
{
    if (index >= sizeVectPos()) {
        throw std::logic_error(
            "leph::Model::getNameDOF: Invalid index.");
    }
...
const std::string& Model::getNameJoint(size_t index) const
{
    if (index >= sizeJoint()) {
        throw std::logic_error(
            "leph::Model::getNameJoint: Invalid index.");
    }
...
const std::string& Model::getNameFrame(size_t index) const
{
    ...
    throw std::logic_error(
        "leph::Model::getNameFrame: Invalid index.");
}
...
double Model::getDOFPos(size_t index) const
{
    if (index >= sizeVectPos()) {
        throw std::logic_error(
            "leph::Model::getDOFPos: Invalid index: " 
            + std::to_string(index));
    }
...
void Model::setDOFPos(size_t index, double val)
{
    if (index >= sizeVectPos()) {
        throw std::logic_error(
            "leph::Model::setDOFPos: Invalid index: " 
            + std::to_string(index));
    }
    _position(index) = val;
    _isDirty = true;
...
double Model::getDOFPos(const std::string& name) const
{
    if (_dofNameToIndex.count(name) == 0) {
        throw std::logic_error(
            "leph::Model::getDOFPos: Invalid name: " 
            + name);
    }
...
void Model::setDOFPos(const std::string& name, double val)
{
    if (_dofNameToIndex.count(name) == 0) {
        throw std::logic_error(
            "leph::Model::setDOFPos: Invalid name: " 
            + name);
    }
...
double Model::getDOFVel(size_t index) const
{
    if (index >= sizeVectVel()) {
        throw std::logic_error(
            "leph::Model::getDOFVel: Invalid index: " 
            + std::to_string(index));
    }
...
void Model::setDOFVel(size_t index, double val)
{
    if (index >= sizeVectVel()) {
        throw std::logic_error(
            "leph::Model::setDOFVel: Invalid index: " 
            + std::to_string(index));
    }
    _velocity(index) = val;
    _isDirty = true;
...
double Model::getDOFVel(const std::string& name) const
{
    if (_dofNameToIndex.count(name) == 0) {
        throw std::logic_error(
            "leph::Model::getDOFVel: Invalid name: " 
            + name);
    }
...
void Model::setDOFVel(const std::string& name, double val)
{
    if (_dofNameToIndex.count(name) == 0) {
        throw std::logic_error(
            "leph::Model::setDOFVel: Invalid name: " 
            + name);
    }
...
void Model::setJointPosVect(const Eigen::VectorXd& vect)
{
    if ((size_t)vect.size() != sizeJoint()) {
        throw std::logic_error(
            "leph::Model::setJointPosVect: Invalid vector size.");
    }
...
void Model::setJointVelVect(const Eigen::VectorXd& vect)
{
    if ((size_t)vect.size() != sizeJoint()) {
        throw std::logic_error(
            "leph::Model::setJointVelVect: Invalid vector size.");
    }
...
void Model::setDOFPosVect(const Eigen::VectorXd& vect)
{
    if ((size_t)vect.size() != sizeVectPos()) {
        throw std::logic_error(
            "leph::Model::setDOFPosVect: Invalid vector size.");
    }
    _position = vect;
    _isDirty = true;
}
...
Eigen::Vector3d Model::getBasePos() const
{
    if (_position.size() < 3) {
        throw std::logic_error(
            "leph::Model::getBasePos: Not initialized.");
    }
...
void Model::setBasePos(const Eigen::Vector3d& pos)
{
    if (_position.size() < 3) {
        throw std::logic_error(
            "leph::Model::setBasePos: Not initialized.");
    }
...
Eigen::Quaterniond Model::getBaseQuat() const
{
    if (_position.size() < 7) {
        throw std::logic_error(
            "leph::Model::getBaseQuat: Not initialized.");
    }
...
void Model::setBaseQuat(const Eigen::Quaterniond& quat)
{
    if (_position.size() < 6) {
        throw std::logic_error(
            "leph::Model::setBaseQuat: Not initialized.");
    }
...
Eigen::Vector3d Model::getBaseVelLinear() const
{
    if (_velocity.size() < 6) {
        throw std::logic_error(
            "leph::Model::getBaseVelLinear: Not initialized.");
    }
...
void Model::setBaseVelLinear(const Eigen::Vector3d& vel)
{
    if (_velocity.size() < 6) {
        throw std::logic_error(
            "leph::Model::setBaseVelLinear: Not initialized.");
    }
...
Eigen::Vector3d Model::getBaseVelAngular() const
{
    if (_velocity.size() < 6) {
        throw std::logic_error(
            "leph::Model::getBaseVelAngular: Not initialized.");
    }
...
void Model::setBaseVelAngular(const Eigen::Vector3d& vel)
{
    if (_velocity.size() < 6) {
        throw std::logic_error(
            "leph::Model::setBaseVelAngular: Not initialized.");
    }
...
void Model::setDOFVelVect(const Eigen::VectorXd& vect)
{
    if ((size_t)vect.size() != sizeVectVel()) {
        throw std::logic_error(
            "leph::Model::setDOFVelVect: Invalid vector size.");
    }
    _velocity = vect;
    _isDirty = true;
}
    ...
    const Eigen::Vector3d& posInWorld)
{
    ...
    if (_isDirty) {
        throw std::logic_error(
            "leph::Model::setBaseToMatchFramePosition: "
            "Internal model is not updated.");
    }
    ...
    const Eigen::Quaterniond& quatInWorld)
{
    ...
    if (_isDirty) {
        throw std::logic_error(
            "leph::Model::setBaseToMatchFrameOrientation: "
            "Internal model is not updated.");
    }
    ...
    const Eigen::Quaterniond& quatInWorld)
{
    ...
    if (_isDirty) {
        throw std::logic_error(
            "leph::Model::setBaseToMatchFramePose: "
            "Internal model is not updated.");
    }
    ...
    const Eigen::Vector6d& velocityInWorld)
{
    ...
    if (_isDirty) {
        throw std::logic_error(
            "leph::Model::setBaseVeltoMatchFrameVel: "
            "Internal model is not updated.");
    }
...
Eigen::VectorXd Model::computeGravityVector()
{
    ...
    if (_isDirty) {
        throw std::logic_error(
            "leph::Model::computeGravityVector: "
            "Internal model is not updated.");
    }
    ...
    Eigen::VectorXd* G)
{
    ...
    if (_isDirty) {
        throw std::logic_error(
            "leph::Model::computeEquationOfMotion: "
            "Internal model is not updated.");
    }
    ...
    Eigen::Vector6d* wrench)
{
    ...
    if (_isDirty) {
        throw std::logic_error(
            "leph::Model::computeJointTorqueSingleContact: "
            "Internal model is not updated.");
    }
    ...
    Eigen::VectorXd& CMMDotQDot)
{
    ...
    if (_isDirty) {
        throw std::logic_error(
            "leph::Model::computeCentroidalDynamics: "
            "Internal model is not updated.");
    }
    //Check vector sizes
    if (
    ...
    const Eigen::VectorXd& torque)
{
    ...
    if ((size_t)position.size() != sizeVectPos()) {
        throw std::logic_error(
            "leph::Model::forwardDynamicsContacts: "
    ...
    const Eigen::VectorXd& torque)
{
    ...
    if ((size_t)position.size() != sizeVectPos()) {
        throw std::logic_error(
            "leph::Model::forwardImpulsiveDynamicsContacts: "
            "Invalid position vector size.");
    }
    if ((size_t)velocity.size() != sizeVectVel()) {
        throw std::logic_error(
