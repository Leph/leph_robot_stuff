#ifndef LEPH_MATHS_AXISANGLE_H
#define LEPH_MATHS_AXISANGLE_H
...
inline Eigen::Vector3d MatrixToAxis(const Eigen::Matrix3d& mat)
{
    ...
    if (std::fabs(val) > 1.1) {
        throw std::logic_error("leph::MatrixToAxis: Invalid input matrix");
    }
    ...
    if (std::isnan(theta)) {
        throw std::logic_error("leph::MatrixToAxis: Error NaN");
    }
