#ifndef LEPH_MATHS_CLAMP_H
#define LEPH_MATHS_CLAMP_H
...
inline T ClampAbsolute(T value, T max)
{
    if (max < 0.0) {
        throw std::logic_error(
            "leph::ClampAbsolute: Invalid absolute bound.");
    }
...
inline T ClampDistFromRange(T value, T min, T max)
{
    if (max < min) {
        throw std::logic_error(
            "leph::ClampDistFromRange: Invalid range.");
    }
...
inline T ClampDistFromAbsolute(T value, T max)
{
    if (max < 0.0) {
        throw std::logic_error(
            "leph::ClampDistFromAbsolute: Invalid absolute bound.");
    }
